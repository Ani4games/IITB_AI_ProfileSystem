...
LOOKUP_RULES: list[dict] = [
...

    {
        ...
        "patterns": [
            r"\b(equipment|machine).{0,20}(request|booking|usage)\b",
            r"\bhow many.{0,20}(equipment|machine).{0,20}(request|booking|usage)\b",
            r"\bequipment usage\b",
            r"\bslot.{0,10}(booked|booking)\b",
            r"\btool.{0,15}(request|usage|booking)\b",
            r"\bhow many.{0,20}(request|booking).{0,10}(made|submitted|placed)\b",
            r"\b(machine|equipment).{0,15}(booked|booking|used)\b",
        ],
        "fields":   ["eq_requests"],
        "formatter": lambda ctx: (
