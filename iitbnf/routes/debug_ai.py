...
def debug_query(profile_type, profile_id):
    ...
    t0     = time.perf_counter()
    raw    = retrieve(query, k=RAG_K) if index_size > 0 else []
    ms     = round((time.perf_counter() - t0) * 1000, 2)
...
def debug_prompt(profile_type, profile_id):
    ...
    query  = _build_report_query(ctx)
    raw    = retrieve(query, k=RAG_K) if collection_size() > 0 else []
    chunks = [c for c in raw if c.get("score", 0) >= MIN_SCORE]
...
def debug_staff(member_id):
    ...
    from models.staff import (
        ...
        get_equipment_stats, get_monthly_reports,
        get_profile_tracking, get_staff_reservations,
        get_staff_system_owned, get_staff_owner_track,
        get_staff_tool_perms_rich, get_project_data
    )
    ...
    report["sections"]["monthly_reports"] = check("monthly",   get_monthly_reports,   member_id, year)
    report["sections"]["profile_tracking"]= check("tracking",  get_profile_tracking,  member_id, year)
    ...
    report["sections"]["projects"]        = check("projects",   get_project_data,      member_id)
...
def debug_lab(memberid):
    ...
    report["sections"]["tool_perms"]    = check("tool_perms",   get_lab_tool_permissions,  memberid)
    ...
    report["sections"]["projects"]      = check("projects",     _get_lab_projects,         memberid)
